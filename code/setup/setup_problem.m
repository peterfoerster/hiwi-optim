function [problem_data, method_data] = setup_problem (geometry_file, order)
    problem_data.geo_name     = [geometry_file '.txt'];
    problem_data.drchlt_sides = [1 2 3];
    problem_data.nmnn_sides   = [4];

    epsilon_r = 9.4;
    problem_data.epsilon = @(x,y,ip) epsilon(ip, x, y, epsilon_r, geometry_file, order);
