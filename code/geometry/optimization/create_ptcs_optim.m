function [ptcs] = create_ptcs_optim (ptcs, order, crv)
   bnds     = nrbextract(ptcs(9));
   bnds(3)  = nrbline(nrbeval(bnds(1), 0), nrbeval(crv(9), 0));
   bnds(4)  = nrbline(nrbeval(bnds(1), 1), nrbeval(crv(9), 1));
   ptcs(9)  = nrbcoons(bnds(3), bnds(4), bnds(1), crv(9));
   bnds     = nrbextract(ptcs(11));
   ptcs(11) = nrbcoons(bnds(3), bnds(4), bnds(1), nrbdegelev(bnds(2), order-3));
   ...
   ptcs(10) = nrbcoons(bnds(3), bnds(4), bnds(1), nrbdegelev(bnds(2), order-3));
