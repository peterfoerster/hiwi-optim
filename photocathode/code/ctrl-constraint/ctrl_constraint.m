function [f_c] = ctrl_constraint(x, order, N_ctrl)
   ...
   % only use minimum difference as constraint function, should give reasonable smoothness
   % provided the changes in x are small enough

   % NLopt needs to have inverse behavior
   f_c = -min([min(ctrl_diff_lb) min(ctrl_diff_ub)]);
end
