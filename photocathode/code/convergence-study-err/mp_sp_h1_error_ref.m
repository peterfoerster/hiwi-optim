...
function [errh1, errl2, errh1s] = mp_sp_h1_error_ref (space_ref, msh_ref, u_ref, u_conv, space_conv, geometry, filename)
...
for iptc = 1:msh_ref.npatch
  ...
  % single patch
  [error_h1(iptc), error_l2(iptc), error_h1s(iptc)] = sp_h1_error_ref (msh_ref.msh_patch{iptc}, space_ref.sp_patch{iptc}, u_ptc_ref, u_ptc_conv, space_conv.sp_patch{iptc}, geometry(iptc), iptc, filename);
end
