function [nrb_optim, knts] = create_nrb_optim (ptcs, order)
   ...
   % glue nurbs
   % this leads to additional knots which would lead to more patches in the cutting process
   ...
   switch(order)
      case{3}
         knts = [zeros(1,order) pt(9) pt(8) pt(7) pt(6) ones(1,order)];
      case{4}
         off  = [-1e-2 1e-2];
         knts = [zeros(1,order) pt(9)+off pt(8)+off pt(7)+off pt(6)+off ones(1,order)];
      case{5}
         off  = [-1e-2 0 1e-2];
         knts = [zeros(1,order) pt(9)+off pt(8)+off pt(7)+off pt(6)+off ones(1,order)];
      case{6}
         off  = [-1e-2 -1e-1 1e-1 1e-2];
         knts = [zeros(1,order) pt(9)+off pt(8)+off pt(7)+off pt(6)+off ones(1,order)];
      case{7}
         off  = [-1e-2 -1e-1 0 1e-1 1e-2];
         knts = [zeros(1,order) pt(9)+off pt(8)+off pt(7)+off pt(6)+off ones(1,order)];
   end
