...
%   epsilon: cell array of function handles to compute the coefficients. Equal to one if left empty.
...
function A = op_gradu_gradv_mp_aniso (spu, spv, msh, coeff, patch_list)
  ...
  for iptc = patch_list
    if (nargin < 4 || isempty (coeff))
      ...
      [rs, cs, vs] = op_gradu_gradv_tp (spu.sp_patch{iptc}, spv.sp_patch{iptc}, msh.msh_patch{iptc}, coeff, iptc);
    end
